...
@section('content')
    <div class="container-fluid">
        <h4 class="page-title mt-4">Edit Blog</h4>
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <div class="card-title">Form Edit Blog</div>
                    </div>
                    <form action="{{ route('blog.update', $blog->id) }}" method="POST" enctype="multipart/form-data">
                                ...
                                <div class="col-md-8">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control @error('judul') is-invalid @enderror"
                                               name="judul" value="{{ old('judul', $blog->judul) }}" required>
                                        @error('judul')
                                    ...
                                    <div class="form-group">
                                        <label for="deskripsi">Deskripsi/Ringkasan <span class="text-danger">*</span></label>
                                        <textarea class="form-control @error('deskripsi') is-invalid @enderror"
                                                  name="deskripsi" rows="3" required>{{ old('deskripsi', $blog->deskripsi) }}</textarea>
                                        <small class="form-text text-muted">Ringkasan singkat yang akan ditampilkan di daftar blog</small>
                                        @error('deskripsi')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control @error('isi_blog') is-invalid @enderror"
                                                  name="isi_blog" id="editor" rows="15" required>{{ old('isi_blog', $blog->isi_blog) }}</textarea>
                                        @error('isi_blog')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        @if($blog->gambar)
                                            <div class="mb-2">
                                                ...
                                                <br>
                                                <img src="{{ $blog->gambar_url }}" alt="Current Image" class="img-thumbnail" style="max-width: 200px;">
                                            </div>
                                        @endif
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="date" class="form-control @error('tanggal') is-invalid @enderror"
                                               name="tanggal" value="{{ old('tanggal', $blog->tanggal->format('Y-m-d')) }}" required>
                                        @error('tanggal')
                                            <div class="invalid-feedback">{{ $message }}</div>
                                        @enderror
                                    </div>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control @error('kategori') is-invalid @enderror" name="kategori" required>
                                            ...
                                            @foreach($kategoris as $key => $value)
                                                <option value="{{ $key }}" {{ old('kategori', $blog->kategori) == $key ? 'selected' : '' }}>
                                                    {{ $value }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="alert alert-info">
                                        ...
                                        <small>
                                            • Penulis: <strong>{{ $blog->user->name ?? 'Unknown' }}</strong><br>
                                            • Slug: <code>{{ $blog->slug }}</code><br>
                                            • Dibuat: {{ $blog->created_at->format('d M Y H:i') }}<br>
                                            • Diupdate: {{ $blog->updated_at->format('d M Y H:i') }}
                                        </small>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="card-footer">
                            ...
                            <a href="{{ route('blog.show', $blog->id) }}" class="btn btn-info">
                                <i class="fa fa-eye"></i> Lihat Blog
                            </a>
